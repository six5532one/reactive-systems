...
object Async {
  ...
  def concurrentComputations[A, B](
    ...
    val result1 = makeAsyncComputation1()
    val result2 = makeAsyncComputation2()
    result1.zip(result2)
  }
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] =
    {
      def f(attemptCount: Int): Future[A] =
        if (attemptCount == maxAttempts)
          makeAsyncComputation()
        else
          makeAsyncComputation().recoverWith( {case t: Throwable => f(attemptCount+1)} )

      if (maxAttempts < 1)
        Future(throw new IllegalArgumentException("Must attempt one or more times"))
      else
        f(1)
    }
